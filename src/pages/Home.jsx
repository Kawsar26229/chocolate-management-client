...
import { FaArrowLeft, FaCircleXmark, FaPencil } from "react-icons/fa6";
import Swal from "sweetalert2";
import { useState } from "react";

const Home = () => {
  const laodedChocolates = useLoaderData();
  const [chocolates, setChocolates] = useState(laodedChocolates)
  const handleDelete = (id) => {
    ...
    .then(data => {
        if(data.deletedCount>0) {
            const remaining = laodedChocolates.filter(choco => choco._id !== id)
  ...
  return (
    <div className="mt-6">
      <Link to="/addChocolate" className="btn btn-outline btn-ghost my-6">
        <FaArrowLeft /> New Chocolate{" "}
        <svg
          ...
          <tbody>
              {chocolates.map((chocolate) => (
                <tr key={chocolate._id}>
                  ...
                  <td>
                    <button className="btn bg-orange-100 me-2">
                      <FaPencil />
                    </button>
                    <button onClick={() => handleDelete(chocolate._id)} className="btn bg-orange-100">
                      <FaCircleXmark />
                    </button>
                  </td>
                </tr>
